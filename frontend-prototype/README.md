...
移动端使用单泳道看板，避免页面级横向溢出；Chat 与 Inbox 使用列表和详情二选一的导航模式，并提供明确返回入口。命令面板支持组合框语义、方向键选择、Enter 打开与 Esc 关闭。
...
- 认证、工作区、Issue 创建/拖拽、评论、Chat、Autopilot、Settings 与各页面主操作；
- CSP、离线字体、恶意标题转义和表单字段白名单；
- 24 个页面路由在 1440×900 / 390×844、亮色 / 暗色下的 96 组合布局；
- 手机端无页面级横向溢出，单泳道看板与 Chat/Inbox 往返可达；
- `app.js` V8 函数、行与字节覆盖率均不低于 90%。

需要生成逐页视觉证据时，在同一 Chrome 会话下运行：

```bash
npm run test:visual
```

脚本会在 `visual-artifacts/` 输出 96 张真实浏览器截图，供人工逐页视觉审批。该目录已忽略，不进入提交；测试会先刷新到干净状态，避免交互用例产生的数据污染截图。当前脚本不做自动像素基线比对，因此截图计数本身不代表像素一致性已获批准。
