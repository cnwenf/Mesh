...
async def setup_world(api_client: httpx.AsyncClient, suffix: str) -> dict:
    ...
    return {
        ...
        "agent_id": agent_resp.json()["data"]["id"], "member_id": member_id,
    }
...
async def _make_slack_world(api_client, suffix, team="T_E2E"):
    ...
    resp = await api_client.post(
        ...
        json={
            "kind": "im_slack", "name": f"slack-{suffix}",
            "config": {
                "team_id": team,
                "bot_user_id": "U_BOT",
    ...
    assert resp.status_code == 201, resp.text
    world["integration"] = resp.json()["integration"]
    ...
    bind = await api_client.post(
        ...
        json={"external_ref": "C_E2E", "bound_agent_id": world["agent_id"],
...
def _slack_mention_payload(team: str, ts_str: str) -> dict:
    return {
        ...
        "event": {"type": "message", "channel": "C_E2E", "user": "U_HUMAN",
                  "text": "<@U_BOT> 帮忙看一下", "event_ts": ts_str},
    }
        ...
        async with session_factory() as session:
            return (await session.execute(
                select(TaskExecution).where(
                    TaskExecution.idempotency_key == expected_key
                )
            )).scalars().first()
    ...
    async with session_factory() as session:
        count = (await session.execute(
            select(TaskExecution.id).where(
                TaskExecution.workspace_id == uuid.UUID(world["ws_id"])
            )
        )).all()
...
async def test_invalid_missing_replay_never_dispatch(api_client, integrations_worker, session_factory):
    ...
    bodies = [
        json.dumps(_slack_mention_payload("T_E2E", f"333.000{i}")).encode()
        for i in range(3)
    ]
    # invalid signature
    bad = await api_client.post(
        "/api/v1/integrations/slack/events", content=bodies[0],
    ...
    missing = await api_client.post(
        "/api/v1/integrations/slack/events", content=bodies[1],
        headers={"content-type": "application/json"},
    )
    assert missing.status_code == 401
    # replay window: valid signature with stale timestamp
    stale = await api_client.post(
        "/api/v1/integrations/slack/events", content=bodies[2],
    ...
    async with session_factory() as session:
        rows = (await session.execute(
            select(IntegrationEvent).where(
                IntegrationEvent.workspace_id == uuid.UUID(world["ws_id"])
            )
        )).scalars().all()
...
async def test_disabled_integration_rejects(api_client, integrations_worker):
    ...
    patch = await api_client.patch(
        ...
        json={"status": "disabled"}, headers=_auth(world["token"]),
    )
...
async def test_feishu_challenge_echo(api_client):
    ...
    resp = await api_client.post(
        f"/api/v1/workspaces/{world['ws_id']}/integrations",
        json={"kind": "im_feishu", "name": "fs-chal", "config": {
            "tenant_key": "tk-chal",
    ...
    challenge_body = json.dumps({
        "type": "url_verification", "challenge": "mesh-challenge-1",
...
async def test_t29_cross_workspace_binding_conflict(api_client):
    ...
    for world in (world_a, world_b):
        resp = await api_client.post(
            f"/api/v1/workspaces/{world['ws_id']}/integrations",
            json={"kind": "im_slack", "name": f"slack-{world['ws_id'][:6]}",
    ...
    bind_a = await api_client.post(
        ...
        json={"external_ref": "C_SHARED"}, headers=_auth(world_a["token"]),
    )
    ...
    bind_b = await api_client.post(
        ...
        json={"external_ref": "C_SHARED"}, headers=_auth(world_b["token"]),
    )
...
async def test_t29_scope_xor_and_project_cascade(api_client, session_factory):
    ...
    project = (await api_client.post(
        ...
        json={"name": "P-XOR", "key": "PX"}, headers=_auth(world["token"]),
    )).json()["data"]
    ...
    async with session_factory() as session:
        remaining = (await session.execute(text(
            "SELECT count(*) FROM integration_bindings WHERE external_ref = 'C_PROJ'"
        ))).scalar_one()
...
async def test_t29_identity_multi_workspace_and_origin_delete(
    ...
    ws_a = (await api_client.post(
        "/api/v1/workspaces", json={"name": "MW A", "slug": "mw-a"}, headers=_auth(token),
    )).json()["data"]["id"]
    ws_b = (await api_client.post(
        "/api/v1/workspaces", json={"name": "MW B", "slug": "mw-b"}, headers=_auth(token),
    )).json()["data"]["id"]
    for ws in (ws_a, ws_b):
        integ = await api_client.post(
            f"/api/v1/workspaces/{ws}/integrations",
            json={"kind": "im_slack", "name": f"slack-mw-{ws[:6]}",
                  "config": {"team_id": "T_MULTI"}},
            headers=_auth(token),
        )
        assert integ.status_code == 201
    integ_a = (await api_client.get(
        f"/api/v1/workspaces/{ws_a}/integrations", headers=_auth(token)
    )).json()["data"][0]
    integ_b = (await api_client.get(
        f"/api/v1/workspaces/{ws_b}/integrations", headers=_auth(token)
    )).json()["data"][0]
    ...
    link = await api_client.post(
        ...
        json={"provider": "slack", "integration_id": integ_a["id"],
              "external_user_key": "U_MULTI"},
        headers=_auth(token),
    )
    ...
    dup = await api_client.post(
        ...
        json={"provider": "slack", "integration_id": integ_b["id"],
              "external_user_key": "U_MULTI"},
        headers=_auth(token),
    )
    ...
    delete_a = await api_client.request(
        "DELETE", f"/api/v1/workspaces/{ws_a}",
        headers=_auth(token), json={"confirm_slug": "mw-a"},
    )
    ...
    listed_b = await api_client.get(
        f"/api/v1/workspaces/{ws_b}/external-identities", headers=_auth(token)
    )
    ...
    async with session_factory() as session:
        ...
        policies = (await session.execute(text(
            "SELECT 1 FROM pg_policies WHERE tablename='external_identities'"
        ))).first()
        assert policies is None, "external_identities must have NO workspace RLS (T29⑩)"
        cascade = (await session.execute(text("""
    ...
    ws_c = (await api_client.post(
        "/api/v1/workspaces", json={"name": "MW C", "slug": "mw-c"},
...
async def test_vcs_flow_pr_merged_auto_status(api_client, integrations_worker, session_factory):
    ...
    integ = (await api_client.post(
        f"/api/v1/workspaces/{world['ws_id']}/integrations",
        json={"kind": "vcs_github", "name": "gh-flow", "config": {
    ...
    issue = (await api_client.post(
        ...
        json={"title": "ship it"}, headers=_auth(world["token"]),
    )).json()["data"]
    ...
    payload = {
        ...
        "sender": {"login": "dev"},
        "pull_request": {
            "number": 42, "title": f"{identifier} ship the thing",
            "state": "closed", "merged": True,
            "merged_at": "2026-07-29T10:00:00Z", "head": {"ref": "feature/ship"},
        },
    }
    body = json.dumps(payload).encode()
    sig = hmac.new(webhook_secret.encode(), body, hashlib.sha256).hexdigest()
    resp = await api_client.post(
        "/api/v1/integrations/github/events", content=body,
        headers={"x-hub-signature-256": f"sha256={sig}",
                 "x-github-event": "pull_request", "x-github-delivery": f"del-{uuid.uuid4().hex[:10]}",
                 "content-type": "application/json"},
    )
    assert resp.status_code == 200, resp.text
    ...
    async with session_factory() as session:
        links = (await session.execute(
            select(VcsLink).where(VcsLink.mesh_entity_id == issue_uuid)
        )).scalars().all()
        assert len(links) == 1
        ...
        comment_count = (await session.execute(text(
            "SELECT count(*) FROM comments WHERE issue_id = :iid AND author_kind = 'system'"
        ), {"iid": issue_uuid})).scalar_one()
    ...
    # issue sidebar endpoint lists the link
    sidebar = await api_client.get(
        f"/api/v1/issues/{issue['id']}/vcs-links", headers=_auth(world["token"])
    )
    assert sidebar.status_code == 200
    assert len(sidebar.json()["data"]) == 1
...
async def test_outbound_ledger_retry_and_breaker(api_client, integrations_worker, session_factory):
    ...
    created = await api_client.post(
        ...
        json={"url": "https://nonexistent-e2e-host.invalid/hook",
    ...
    issue = await api_client.post(
        f"/api/v1/workspaces/{world['ws_id']}/issues",
        json={"title": "trigger outbound"}, headers=_auth(world["token"]),
    )
    ...
    await api_client.post(
        f"/api/v1/workspaces/{world['ws_id']}/issues",
        json={"title": "trigger outbound 2"}, headers=_auth(world["token"]),
    )
    ...
    deliveries = (await api_client.get(
...
async def test_outbound_https_only_and_ssrf(api_client):
    ...
    http_url = await api_client.post(
        f"/api/v1/workspaces/{world['ws_id']}/webhook-subscriptions",
        json={"url": "http://plain.example.com/x"}, headers=_auth(world["token"]),
    )
    ...
    private = await api_client.post(
        f"/api/v1/workspaces/{world['ws_id']}/webhook-subscriptions",
        json={"url": "https://10.1.2.3/x"}, headers=_auth(world["token"]),
    )
