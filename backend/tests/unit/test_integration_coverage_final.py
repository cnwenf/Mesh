...
def test_vcs_action_gitlab_variants():
    merged = NormalizedEvent(
        external_event_id="e", event_type="Merge Request Hook",
        external_ref="a/b", actor_key="u", tenant_key="t", text="",
...
def test_external_object_for_variants():
    def event(event_type, external_ref="acme/web", **extra):
        return NormalizedEvent(
            external_event_id="e", event_type=event_type,
            external_ref=external_ref, actor_key="u", tenant_key="t",
            text="", extra=extra,
        )

    assert vl.external_object_for("github", event("push", ref="refs/heads/main")) == (
        "branch", "acme/web@main"
    )
...
async def _world_with_statuses(session_factory):
    ...
    async with session_factory() as session, session.begin():
        ...
        await session.flush()
        issue = Issue(
            workspace_id=world["ws"], identifier_namespace_key="WEB", number=1,
            identifier="WEB-1", title="t", status_id=todo.id, state_category="todo",
        )
        session.add(issue)
...
async def test_strict_mode_blocks_transition(session_factory):
    ...
    async with session_factory() as session, session.begin():
        ...
        ok = await vl.apply_auto_status(
            session, workspace_id=world["ws"], issue=issue, target="done",
            action="merged", external_ref="acme/web#1", event_row=event_row, now=NOW,
        )
        assert ok is True
    ...
    async with session_factory() as session, session.begin():
        ...
        blocked = await vl.apply_auto_status(
            session, workspace_id=world["ws"], issue=reloaded, target="done",
            action="merged", external_ref="acme/web#1", event_row=event_row, now=NOW,
        )
...
async def test_find_target_status_missing(session_factory):
    ...
    async with session_factory() as session, session.begin():
        ...
        result = await vl.apply_auto_status(
            session, workspace_id=world["ws"], issue=issue, target="nonexistent",
            action="merged", external_ref="a/b#1", event_row=event_row, now=NOW,
        )
...
async def _event_row(session_factory, world):
    async with session_factory() as session, session.begin():
        ...
        row = IntegrationEvent(
            ...
            external_event_id=f"evt-{uuid.uuid4().hex[:8]}", event_type="pull_request",
...
async def test_worker_run_forever_processes_and_stops(session_factory):
    ...
    async with session_factory() as session, session.begin():
        ...
        subscription, _ = await ob.create_subscription(
            ...
            url="https://hooks.example.com/x", signing_secret=TEST_SIGNING_SECRET, now=NOW,
        )
        session.add(WebhookSubscriptionDelivery(
            workspace_id=world["ws"], subscription_id=subscription.id,
            event_ref="loop-1", state="pending",
        ))
    ...
    worker = ob.WebhookDeliveryWorker(
        ...
        poll_interval=0.05, clock=lambda: NOW,
    )
    stop = asyncio.Event()
    ...
    await asyncio.wait_for(
        asyncio.gather(worker.run_forever(stop), stopper()), timeout=10
    )
...
async def test_retry_non_failed_delivery_rejected(session_factory):
    ...
    async with session_factory() as session, session.begin():
        ...
        subscription, _ = await ob.create_subscription(
            session, workspace_id=world["ws"], creator_member_id=world["member"],
            url="https://hooks.example.com/y", signing_secret=TEST_SIGNING_SECRET, now=NOW,
        )
        delivery = WebhookSubscriptionDelivery(
            workspace_id=world["ws"], subscription_id=subscription.id,
            event_ref="r-1", state="pending",
        )
...
async def test_service_event_counts_and_filters(session_factory):
    ...
    await service.create_integration(
        workspace_id=world["ws"], creator=member, kind="im_slack", name="svc-x"
    )
    ...
    count = await service.event_counts_7d(
        workspace_id=world["ws"], integration_id=target.id,
    ...
    page2 = await service.list_integrations(
        workspace_id=world["ws"], limit=10, cursor=page1.next_cursor
    )
...
async def test_service_update_binding_clears_agent(session_factory):
    ...
    rendered = await service.create_binding(
        ...
        external_ref="C_CLR", bound_agent_id=world["agent"],
    )
    updated = await service.update_binding(
        workspace_id=world["ws"], binding_id=uuid.UUID(rendered["id"]),
        bound_agent_id=None,
    )
...
async def test_service_foreign_workspace_404(session_factory):
    ...
    with pytest.raises(NotFoundError):
        await service.get_integration(
            workspace_id=uuid.uuid4(), integration_id=world["integ_slack"]
        )
...
async def test_inbound_fallback_without_definer_functions(session_factory):
    ...
    async with session_factory() as session, session.begin():
        ...
        integration.config = {
            **integration.config, "signing_secret_ref": encrypt(signing)
        }
    session = session_factory()
    transaction = await session.begin()
    try:
        await session.execute(text(
            "DROP FUNCTION mesh_integrations_by_kind_config_value(text, text, text)"
        ))
        ...
        payload = {
            ...
            "event": {"type": "message", "channel": "C_FB", "user": "U",
                      "text": "hi", "event_ts": "2.2"},
        }
        ...
        rows = (await session.execute(
            select(IntegrationEvent).where(
                IntegrationEvent.external_event_id == "T_TEST:2.2"
            )
        )).scalars().all()
        assert len(rows) == 1
        # invalid signature through the fallback path → rejected audit row
        bad_body, bad_headers = slack_request("wrong", {**payload, "event": {
            **payload["event"], "event_ts": "3.3"}})
        bad_status, _ = await process_inbound(
            session, kind="im_slack", raw_body=bad_body, headers=bad_headers,
            signing_secret=TEST_SIGNING_SECRET, now=NOW,
            tolerance=timedelta(seconds=300),
        )
