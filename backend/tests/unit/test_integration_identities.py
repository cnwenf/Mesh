...
async def test_link_confirm_creates_global_mapping(session_factory, redis_client):
    ...
    code = await redis_client.get(
        f"{ids_mod.DEV_OUTBOX_PREFIX}slack:T_TEST:U_CLICK"
    )
    assert code is not None and len(code) == ids_mod.CODE_LENGTH
    member = await _member(session_factory, world["member"])
    async with session_factory() as session, session.begin():
        identity = await ids_mod.confirm_link(
            session, redis=redis_client, workspace_id=world["ws"],
            member=member, provider="slack", code=code,
        )
    ...
    leftover = await redis_client.get(
        f"{ids_mod.CODE_PREFIX}{world['ws']}:{world['member']}:slack"
    )
    ...
    async with session_factory() as session, session.begin():
        with pytest.raises(BusinessRuleError):
            await ids_mod.confirm_link(
                session, redis=redis_client, workspace_id=world["ws"],
                member=member, provider="slack", code="000000",
            )
...
async def test_confirm_without_start_rejected(session_factory, redis_client):
    ...
    async with session_factory() as session, session.begin():
        with pytest.raises(BusinessRuleError):
            await ids_mod.confirm_link(
                session, redis=redis_client, workspace_id=world["ws"],
                member=member, provider="slack", code="123456",
            )
...
async def test_unlink_allowed_only_for_owner(session_factory):
    ...
    async with session_factory() as session, session.begin():
        other_user = User(
            id=uuid.uuid4(), email=f"other-{uuid.uuid4().hex[:8]}@mesh.test",
            display_name="Other Admin", password_hash="unused",
        )
        session.add(other_user)
        await session.flush()
        other_admin = Member(
            id=uuid.uuid4(), workspace_id=world["ws"], member_type="human",
            user_id=other_user.id, role="admin", status="active",
        )
        session.add(other_admin)
        await session.flush()
        identity = ExternalIdentity(
            provider="slack", provider_tenant_key="T_TEST",
            external_user_key="U_OWNER", user_id=world["user"],
            created_in_workspace_id=world["ws"],
        )
        session.add(identity)
    async with session_factory() as session:
        assert await ids_mod.external_identity_unlink_allowed(
            session, identity_id=identity.id, member_id=world["member"]
        ) is True, "owner via their member row → allowed"
        assert await ids_mod.external_identity_unlink_allowed(
            session, identity_id=identity.id, member_id=other_admin.id
        ) is False, "admin who is NOT the owner → denied (no bypass)"
...
async def test_unlink_by_other_user_forbidden(session_factory):
    ...
    async with session_factory() as session, session.begin():
        other_user = User(
            id=uuid.uuid4(), email=f"o2-{uuid.uuid4().hex[:8]}@mesh.test",
            display_name="O2", password_hash="unused",
        )
        session.add(other_user)
        await session.flush()
        other_admin = Member(
            id=uuid.uuid4(), workspace_id=world["ws"], member_type="human",
            user_id=other_user.id, role="owner", status="active",
        )
        session.add(other_admin)
        await session.flush()
        identity = ExternalIdentity(
            provider="slack", provider_tenant_key="T_TEST",
            external_user_key="U_MINE", user_id=world["user"],
            created_in_workspace_id=world["ws"],
        )
        session.add(identity)
    async with session_factory() as session, session.begin():
        with pytest.raises(ForbiddenError) as excinfo:
            await ids_mod.unlink_identity(
                session, workspace_id=world["ws"], member=other_admin,
...
async def test_owner_unlink_succeeds_and_audits(session_factory):
    ...
    async with session_factory() as session, session.begin():
        identity = ExternalIdentity(
            provider="slack", provider_tenant_key="T_TEST",
            external_user_key="U_GONE", user_id=world["user"],
            created_in_workspace_id=world["ws"],
        )
        session.add(identity)
    member = await _member(session_factory, world["member"])
    async with session_factory() as session, session.begin():
        await ids_mod.unlink_identity(
            session, workspace_id=world["ws"], member=member,
            identity_id=identity.id,
        )
    async with session_factory() as session:
        assert await session.get(ExternalIdentity, identity.id) is None
        audits = (await session.execute(
            select(AuditLog).where(AuditLog.action == "external_identity.unlinked")
        )).scalars().all()
        assert len(audits) == 1
...
async def test_list_own_identities_filters_by_user(session_factory):
    ...
    async with session_factory() as session, session.begin():
        other_user = User(
            id=uuid.uuid4(), email=f"o3-{uuid.uuid4().hex[:8]}@mesh.test",
            display_name="O3", password_hash="unused",
        )
        session.add(other_user)
        await session.flush()
        session.add(ExternalIdentity(
            provider="slack", provider_tenant_key="T_TEST",
            external_user_key="U_MINE_A", user_id=world["user"],
        ))
        session.add(ExternalIdentity(
            provider="github", provider_tenant_key="",
            external_user_key="octocat", user_id=other_user.id,
        ))
    member = await _member(session_factory, world["member"])
    async with session_factory() as session:
