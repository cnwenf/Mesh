...
| Worker | `python -m mesh.workers` | Outbox relay (incl. `execution.enqueue` → `task_executions`), realtime projector, realtime + outbox retention purges, invitation sweep, attachment quarantine pipeline, **runtime reaper** (lease-expiry reclaim + heartbeat-loss offline + approval expiry), outbound webhook delivery worker, **`im-send-relay`** (integrations.md §3.8/§3.10: sole `im.send` consumer — ack at-most-once T1/T2 protocol, notification chunk delivery with `notification_delivery` ledger writeback, command feedback, approval-card push; the general relay never claims `im.send`) — each an isolated asyncio task under a 1s liveness watchdog; exception, unexpected cancellation, and unexpected return emit structured errors and restart with per-task backoff (§2.2) |
...
- `integrations/` — integrations platform (`docs/specs/features/integrations.md`, README §6.17): unified third-party abstraction (IM/VCS/developer webhook) with five connector adapters (feishu/slack/dingtalk/github/gitlab: signature verification + payload normalization), the DingTalk enterprise-app robot dual receive modes (MES-87: HTTP callback `timestamp`+`sign` verification — `Base64(HMAC_SHA256(app_secret, timestamp+"
"+app_secret))`, constant-time compare, official ±3600s window never narrowed — and Mesh-dialed Stream long-connection: `connections/open` channel auth, `specVersion='1.0'` frames with mandatory ping ACK / disconnect reconnect, wss-only + forced TLS verification, advisory-lock single-instance mutex, per-app_key connection sharing, 2→300s ±20% backoff, un-ACKed redelivery idempotent via msgId; `MESH_DINGTALK_GATEWAY_BASE` deploy-time test door only) behind the one shared ingestion core `ingest_verified_event()` — both receive modes differ only in the auth adapter in front; the core runs verified envelope → dedup → msgtype matrix (text-only triggering) → command plane (§3.7) → binding match → post-signature semantic guardrails (identity 20/min · conversation 60/min, fail-closed) → same-transaction `integration_message_queue` enqueue (imq_seq advisory lock + lock-order `ack_window_at` + drain-then-switch dispatch-mode snapshot + §3.8 ack leader determination + authoritative pending-depth 50 re-check under the lock; `execution.enqueue` consumers add the R5-2 `trigger='integration'` queue-item guard binding the execution to its item), the ingestion pipeline reusing the autopilot `webhook_events` paradigm (constant-time signature check + ±300s replay window → `rejected:<hash>` audit namespace → `UNIQUE(integration_id, external_event_id)` dedup → binding match → same-transaction `execution.enqueue` with `trigger='integration'` and the `sha256(agent|binding|external_event_id)` idempotency key; payloads enter the agent context only under the §6.15 untrusted root), `external_identities` global identity table (maps onto `users.id`; no `workspace_id` / no workspace RLS; owner-only unlink with no admin bypass — `external_identity_unlink_allowed` executable reference), external-identity link flow (one-time code delivered to the external account's DM; dev Redis dev-outbox), card-callback authorization chain (clicker identity → `users.id` → workspace roster JOIN → §4.4/§6.10 permission re-check → unified approvals forwarding), `vcs_links` truth source (identifier `WEB-123` auto-link + `auto_status_map` status flow with system-comment trail), outbound developer webhooks (`webhook.dispatch` outbox derivation from `realtime.publish` → delivery worker with `Mesh-Signature` HMAC headers, exponential backoff, subscription-level circuit breaker; https-only + SSRF guards), OAuth authorization-code + PKCE (single-use state, ciphertext-only tokens), credential ciphertext contract (`secret_ref`, same as `runtime_credentials`; plaintext shown exactly once; decrypted integration secrets join the `redact_in_logs` blacklist), and `integration.updated` / `integration.event_ingested` realtime events over the single outbox write path. **DingTalk outbound + interactive cards** (§3.8/§3.10/§4.4, MES-89): `dingtalk_api.py` (accessToken multi-replica single-flight — Redis shared cache with TTL jitter + random-owner `SET NX EX` lease + Lua conditional release + follower bounded wait; `token_refresh_busy` is a retryable non-failure that only moves `available_at`; robot message send with rate-limit-code classification (`send.too.fast` etc. + `flowControlledStaffIdList`), token-invalidate-once-retry-once, full-channel redaction), `im_outbound.py` (external user-key encoding `x=<base64url(senderId)>` with structurally disjoint key spaces, ≤15000B markdown chunking, verbosity gating `final_only` default, `notification_delivery(channel='im')` derivation chained onto `notification.fanout`, `IMSendRelay` — the sole `im.send` consumer), `ack.py` (leading-edge window leader election inside the enqueue transaction over lock-ordered `ack_window_at`; at-most-once T1 gate → out-of-transaction send → T2 writeback; `ack_template=''` skips all ack processing), `dingtalk_cards.py` (card_1.0 `createAndDeliver`/update/streaming with approval-derived `outTrackId` + `IM_GROUP`/`IM_ROBOT` openSpaceId + `callbackType`; `sampleActionCard*` forbidden for approval cards by code-path assertion; DingTalk callback chain with staffId-normalized clicker → §4.4 lifecycle writebacks; HTTP callback endpoint with the official ±3600s signature tolerance), diagnostics split (`test-send` = outbound-only, failures are 502 `upstream_error`; `stream-status` = receive-only, the sole 503 `stream_channel_unavailable` surface).
...
- `outbox/` — transactional outbox (§6.6): `emit_event`/`emit_realtime` write in the business transaction; the relay claims one row at a time with `FOR UPDATE SKIP LOCKED`, commits each event independently, bounds lock waits, and backs failed events off through `available_at`; the projector is the ONLY writer of `realtime_events` and allocates per-channel `seq` in the same transaction.
...
- `workers/` — supervisor (shared shutdown signal + 1s terminal-state watchdog + structured death logs + isolated per-task restart backoff/cancel domains), retention purge, invitation expiry sweep, process entrypoint.
...
- **Relay lock/backoff guard** — each outbox event and outbound webhook delivery commits in its own transaction. `MESH_OUTBOX_LOCK_TIMEOUT_SECONDS` defaults to `0.5` (`0` disables); `MESH_OUTBOX_FAILURE_BACKOFF_SECONDS` / `MESH_OUTBOX_FAILURE_BACKOFF_MAX_SECONDS` default to `1` / `60`, and `MESH_OUTBOX_ERROR_BACKOFF_SECONDS` defaults to `1`. These bound maintenance-lock contention and prevent a failing row from hot-looping or pre-locking the rest of a batch.
