#!/usr/bin/env bash
# Generate one-use strong credentials for the isolated MES-160 real stack.
set -euo pipefail

MES160_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
MES160_ENV_FILE="${MES160_DIR}/stack.env"

if [[ -e "${MES160_ENV_FILE}" && "${1:-}" != "--force" ]]; then
...
umask 077
{
  echo '# MES-160 real-stack e2e — generated strong random values; NEVER commit.'
  ...
  echo 'MESH_API_PORT=18520'
  echo 'MESH_WS_PORT=18521'
  echo 'MESH_FRONTEND_PORT=18530'
  echo 'MESH_STORAGE_PORT=19520'
  echo 'MESH_STORAGE_CONSOLE_PORT=19521'
  echo 'MESH_STORAGE_PUBLIC_ENDPOINT=http://127.0.0.1:19520'
  echo 'MESH_APP_BASE_URL=http://127.0.0.1:18530'
} > "${MES160_ENV_FILE}"

echo "Wrote ${MES160_ENV_FILE} (mode $(stat -c '%a' "${MES160_ENV_FILE}" 2>/dev/null || stat -f '%Lp' "${MES160_ENV_FILE}"))."
