...
describe('useTaskStream', () => {
  it('invokes onEvent when an orchestration frame arrives', async () => {
    vi.stubGlobal('fetch', vi.fn(async () => liveStream('event: task.status\nid: 1\ndata: {}\n\n')));
    const onEvent = vi.fn();
  ...
  it('ignores frames outside the five orchestration events', async () => {
    vi.stubGlobal('fetch', vi.fn(async () => liveStream('event: error\nid: 1\ndata: {}\n\n')));
    const onEvent = vi.fn();
  ...
  it('does not connect when url is null or the task is terminal', () => {
    ...
    renderHook(() => useTaskStream({ url: 'http://api/stream', enabled: false, onEvent: () => undefined }));
