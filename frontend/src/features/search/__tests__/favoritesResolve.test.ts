...
vi.mock('../../agents/api', () => ({ getAgent: vi.fn() }));
...
describe('resolveTarget(单目标解析,§4.2.1 步骤 3)', () => {
  it('issue → 标题 + 规范 by-identifier 深链(§3.4)', async () => {
    vi.mocked(getIssue).mockResolvedValue({ id: 'i-1', identifier: 'WEB-7', title: '崩溃' } as never);
  ...
  it('chat_session / member / agent(workspace 路径端点)取各自标题字段', async () => {
    ...
    vi.mocked(getMember).mockResolvedValue({ id: 'm-1', display_name: '成员' } as never);
    expect(await resolveTarget(client, 'member', 'm-1', SCOPE)).toEqual({
      status: 'ok',
      title: '成员',
      url: '/w/acme/members/m-1',
    });
    vi.mocked(getAgent).mockResolvedValue({ id: 'a-1', display_name: '助手' } as never);
    expect(await resolveTarget(client, 'agent', 'a-1', SCOPE)).toEqual({
      status: 'ok',
      title: '助手',
      url: '/w/acme/agents/a-1',
    });
  });
