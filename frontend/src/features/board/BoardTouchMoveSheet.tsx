...
import { Button, Drawer, Icon } from '../../design';
...
export function BoardTouchMoveSheet(props: BoardTouchMoveSheetProps): React.JSX.Element {
  ...
  const handleSelectColumn = useCallback(
    (columnKey: string) => {
      ...
      announce(
        t('board.touchMoved', { identifier: card.identifier, column: getColumnLabel(columnKey) }),
      );
      onClose();
    },
  ...
  const handleReorder = useCallback(
    (direction: 'top' | 'bottom' | 'up' | 'down') => {
      ...
      announce(
        t('board.reorderMoved', {
          identifier: card.identifier,
          direction: t('board.reorder.' + direction),
        }),
      );
    },
  ...
  return (
    ...
    >
      <div className="mesh-board-touch" data-testid="board-touch-sheet">
        {/* 列内排序操作 */}
        <div
          className="mesh-board-touch__reorder"
          role="group"
          aria-label={t('board.reorderGroupLabel')}
        >
          <Button
            variant="secondary"
            size="lg"
            className="mesh-board-touch__reorder-btn"
            data-testid="touch-move-top"
        ...
        {/* 目标列列表 */}
        <ul
          className="mesh-board-touch__columns"
          role="list"
          aria-label={t('board.targetColumnsLabel')}
        >
          {columns.map((column) => {
            ...
            return (
              <li key={column.key}>
                ...
                >
                  ...
                  {column.wip !== null ? (
                    <span className="mesh-board-touch__wip">
                      {column.count}/{column.wip.limit}
                    </span>
                  ) : null}
                  {blocked ? (
                    <span
                      className="mesh-board-touch__blocked-reason"
                      data-testid={`touch-blocked-reason-${column.key}`}
                    >
