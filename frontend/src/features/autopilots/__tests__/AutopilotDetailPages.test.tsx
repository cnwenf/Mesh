...
describe('AutopilotDetailPage', () => {
  it('renders config card and run history', async () => {
    ...
    expect(screen.getByTestId('autopilot-runs-table').tagName).toBe('OL');
    expect(screen.getByTestId('autopilot-run-row-run-1')).toBeInTheDocument();
    expect(screen.getByTestId('autopilot-run-row-run-1').tagName).toBe('BUTTON');
...
describe('WebhookConfigPage', () => {
  it('creates a credential and shows it exactly once', async () => {
    ...
    expect(screen.getByTestId('webhook-create-secret')).toBeDisabled();
    expect(screen.getByTestId('webhook-secrets-table')).not.toHaveTextContent('whs_sec');
    expect(
