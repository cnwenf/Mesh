 ...
 *
 * - 桌面(>720px):grid 双栏,左栏 `minmax(16rem, 20rem)` 列表,右栏详情,两栏各自滚动。
 * - 手机(≤720px,§8.1/§8.3):单栏,经 `activePane` 决定可见窗格——路由化由调用方承担
 *   (如 /chat 与 /chat/:sessionId、/inbox 与 /inbox/:notificationId),本组件不读路由、
 *   不读窗口宽度,纯由 prop 驱动(断点集中于 ConversationLayout.css,与外壳 599px 之外
 *   的会话类专用 720px 阈值,沿用既有 chat 双栏折叠口径)。
 *
