...
- [x] 公开流程冻结为 `384px` 内容框、`16px` 区块/卡片节奏和 `24/32px` 标题，并覆盖桌面、平板、手机的亮暗视觉基线。
- [x] 账号设置采用 `224px + 704px` 双列布局，compact 为顶部横向导航；新增真实个人资料入口，外观/语言/时区合并为紧凑字段行。
- [x] 看板使用横向视图导轨、`280px` 列、`16px` 列距和信息卡；普通卡与虚拟化卡分别维持信息密度和性能契约。
- [x] 工作区首页从极简字段页补齐真实项目、issue、收件箱、执行概览，每个来源有独立 loading/empty/error/ready 状态和规范深链。
- [x] 顶栏用户菜单覆盖个人设置、主题三态（含 system 解析值）、快捷键帮助和退出；评论相对时间每 `30s` 推进并在 tooltip 显示本地时区与 UTC。
- [x] 变更后的正常态、异常态和逐页四组合证据均经 actual/expected/diff 人工复核后更新；测量、差异说明与保留边界见 `docs/audits/mes185-interface-alignment-audit.md`。
