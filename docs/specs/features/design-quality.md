...
`frontend-prototype/` 是页面结构、视觉节奏、主题和响应式交互的可双击蓝图，不替代业务 Spec 或生产 React 实现。蓝图修改必须满足：

- 24 个页面路由均可由 URL hash 直达，页面标题与 Mesh 品牌正确；
- 1440×900 桌面和 390×844 手机在 light/dark 下组成 96 个逐页组合，均由真实 Chrome 渲染；
- 手机端不得出现页面级横向溢出；看板使用单泳道滚动，Chat/Inbox 使用列表与详情二选一导航；
- 命令面板具备 combobox/listbox 语义，并支持方向键、Enter 和 Esc；
- 认证、工作区切换/创建、Issue 创建/拖拽、评论、Chat、Autopilot、主题与页面主操作均由真实浏览器交互覆盖；
- CSP、离线字体、UGC 转义、字段白名单和同源 CDP 限制持续通过；
- `app.js` 的 V8 函数、行与字节覆盖率均不得低于 90%，视觉截图必须在刷新后的干净状态生成。

执行入口：`npm test` 运行交互、安全、四组合结构与覆盖率门禁；`npm run test:visual` 额外输出 96 张逐页截图到忽略目录 `visual-artifacts/`。这些截图是人工逐页视觉审批的证据，不等同于自动像素基线比对；未引入经审批的基线前，不得把截图计数宣称为像素回归通过。
