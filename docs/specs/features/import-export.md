...
- **H2 确定性验收(MES-149,写死)**:受控时钟的 reaper 测试与后台 worker/reaper 必须生命周期隔离,不允许两者竞争同一作业行;第二次重投必须经真实 outbox relay handler 消费。成功条件不得是单次瞬时快照,而必须在同一持久状态轮询中联合收敛为:目标 resume outbox 行 `published` 且 `published_at` 非空/零失败次数;作业精确 `completed` 且租约清空/2 成功 0 失败/`last_committed_batch=1`/`resumed_count=1`;实体与行台账各恰好 2 条。失败通知同样按 `data_job:{id}:finished` 组键轮询目标作业,禁止读取任意第一条通知。上述用例不得使用 `@pytest.mark.flaky` 或失败重跑代替终态收敛。
...
- [ ] **H2 同 checkpoint 双崩溃确定性(MES-149)**:真实 relay 恢复按 §3.8 的联合终态断言一次收敛,失败重跑标记为零;失败通知按目标 job group key 收敛为 critical 且 `preview` 含失败原因,不依赖行顺序或瞬时状态。
