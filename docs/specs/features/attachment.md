...
- **无前缀端点 404 口径(workspace.md §5.3)**:`/attachments/{id}`(及 `/complete`、`/abort`、`/download`、`/thumbnail`)、`/multipart/{id}/parts|complete`、`/issues|comments/{id}/attachments` 均经 SECURITY DEFINER(`mesh_attachment_workspace_id` / `mesh_issue_workspace_id` / `mesh_comment_workspace_id`)解析租户,再过成员门;门的 404 在路由层转写为资源消息(`attachment not found` / `{linked_type} not found`),「id 不存在」「存在但非成员」「软删除」三态消息不可区分,无存在性 oracle。`POST /attachments/upload-requests` 仅 `link_to` 派生租户分支取宿主资源消息;**显式 `workspace_id` 分支与 token 自身工作区属调用方指名,保持 `workspace not found`**(与 `require_workspace` 一致)。`gate_workspace` 为共享件(含 token 调用方),转写只在无前缀调用点包裹,不改其本体。
