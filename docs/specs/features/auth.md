...
- **登录页**:`/login`;邮箱+密码、「记住我」、「忘记密码?」、「使用第三方账号登录」按钮组;注册入口。失败统一提示「邮箱或密码不正确」,不暴露账号是否存在。
...
- **未登录路由守卫(MES-106)**:受保护路由(首页、设置、工作区等 shell 子路由)位于 `RequireAuth` 守卫之后——未登录(无 access token)访问时**不渲染受保护子树、不发起受保护请求**,统一 `Navigate` 到 `/login?next=<原路径(含查询串,经编码)>`,登录成功经上述 `safeNextPath` 守卫回跳原页;公开页(邀请接受预览、登录/注册、忘记密码/重置、OAuth 回调、设备码确认)位于守卫之外。已登录用户访问 `/login` 反向回跳目标页(避免重复登录)。守卫只判「本地无 token」,绝不发探测请求。
- **401 全局兜底(MES-106)**:token 存在但失效(过期/撤销)时受保护端点回 401——API 客户端对**非鉴权豁免端点**的 401 统一执行「清除本地 access token + 整页跳 `/login?next=<当前路径>`」,不再由各组件各自呈现「加载失败」。鉴权豁免端点(登录/注册/MFA 验证/忘记密码/重置/验证邮箱/OAuth 往返)的 4xx 属业务错误,就地呈现具名文案,不触发兜底;已在登录页时仅清 token 不跳转(防成环)。token 一经清除,路由守卫随 store 订阅同步生效,两条路径收敛于同一登录跳转。
- **实时网关地址(MES-106)**:`WebSocket` 构造器仅接受**绝对** URL——同源部署(构建期 `VITE_MESH_WS_BASE_URL` 为空,nginx 反代 `/ws`)时由页面 `location` 派生绝对地址(`https:` 页面 → `wss://<host>/ws`,`http:` 页面 → `ws://<host>/ws`,公网 HTTP 场景据此可用);显式 `http(s)://` 基址归一为 `ws(s)://` scheme。首帧认证(§6.16)不变。
- **注册页**:`/register` 为可刷新、可分享的独立公开入口,直接呈现注册模式；与 `/login` 的注册切换共用同一认证流程。字段为邮箱+显示名+密码(强度条 + 实时校验);提交后跳「已发验证邮件」页(注册成功自动登录态不阻塞,结果页提供「继续」入口回跳)。
