...
**只追加**:不允许 UPDATE/DELETE(合规);**DB 级 enforcement**:应用数据库账号对 `audit_logs` 表仅授予 `INSERT` + `SELECT` 权限(回收 `UPDATE`/`DELETE`),或启用 `BEFORE UPDATE OR DELETE` 触发器拒绝修改;可配合分区归档冷存储。
...
- `/ws` 连接建立时用 token 鉴权(握手携带或首条消息认证),服务端校验后按 `workspace_id + principal` 注册频道;频道订阅 + 单调递增 `seq` + 断线重放(见各模块 Spec 事件表)。**禁止在 URL query 参数中传递 token**(如 `/ws?token=<JWT>`),token 会落入访问日志与中间代理;应使用 WebSocket 子协议(Sec-WebSocket-Protocol)或连接建立后首帧认证。
