...
> 4. **原子改名切换(单一快速事务,只做改名)**:同事务内 `ALTER FUNCTION public.mesh_search_norm RENAME TO mesh_search_norm_prev; ALTER FUNCTION public.mesh_search_norm_next RENAME TO mesh_search_norm;`(查询按名解析 → 自动落到新实现)+ 投影列改名(`search_name_next → search_name`)+ 11 条索引改名(`idx_*_next → idx_*`,旧名让位为 `idx_*_prev`)+ 停双写;**改名不更 OID:新索引绑定新版函数 OID 接管规范名,旧索引仍指向旧函数(现名 `mesh_search_norm_prev`)自然失配**;**事务内只做改名与清理前双侧精确绑定断言(新函数精确绑定 9 条规范索引、旧函数精确绑定 9 条 `_prev`——此时 `_prev` 未删,旧函数依赖理应仍在),不做任何删除/批量写入,事务结束即 COMMIT(快速提交,最小化 ACCESS EXCLUSIVE 窗口)**;
> 5. **COMMIT 后可见状态行为验证**:提交后的可见状态下验证新折叠行为落入投影、投影值新旧一致(回补无遗漏)、前缀查询自然命中规范 pattern 索引(真实选择率下不强制 planner)、trigram 表达式查询可走规范 GIN 索引(表达式匹配可用性);
> 6. **逐条事务外 `DROP INDEX CONCURRENTLY`**:全部 11 条 `idx_*_prev` 以 `DROP INDEX CONCURRENTLY` **在事务外逐条删除**(生产契约要求的并发安全删除,不在任何事务块内);
> 7. **旧函数零依赖断言 + 删除旧函数/列**:11 条 `_prev` 删除完成后,以 `pg_depend` 断言**旧版函数零索引依赖**(9 条表达式索引已全部迁移),随后 `DROP FUNCTION public.mesh_search_norm_prev(TEXT)` + 删旧投影列——**删除成功本身即切换完整性证明**(若仍有索引绑定旧函数,DROP 会被拒绝);
> 8. **清理后行为验证**:重跑前缀与 trigram 查询确认规范索引接管全部查询、新函数行为生效。
>
> 每阶段可独立失败回滚(切换前失败:删 `_next` 对象即复原;切换后失败:反向改名回切)。**上述 8 阶段与验证脚本 `schema_r2_validation.sql` 的 T38 结构逐段对应**(事务外建 `_next` → 快速改名事务 COMMIT → 提交后验证 → 事务外 `DROP INDEX CONCURRENTLY` → 零依赖删旧 → 清理后验证),脚本以真实事务边界实跑(R6-H4:此前单 `DO` 事务塞入改名/删除/批量写/EXPLAIN 的结构已重构)。该函数与索引的 PG16 可执行性 + 归一行为(`José→jose`)+ 前缀查询命中断言 + **本升级路径完整 smoke test**(T38:可观察行为差异的新版函数(词典版本共存)→ 回补 → 事务外建全部 11 条 `_next` 索引 → 原子改名切换(`pg_depend` 双侧精确断言:新函数绑定 9 条规范索引、旧函数绑定 9 条 `_prev`)→ COMMIT 后行为验证 → 事务外逐条 `DROP INDEX CONCURRENTLY` → 旧函数零依赖并删除旧函数/列 → 清理后前缀与 trigram 查询命中新索引)在 `docs/specs/validation/schema_r2_validation.sql`(T37 建表与行为 / T38 升级路径)实跑验证。
...
-- 1b. 1–2 字符前缀:B-tree pattern 索引(workspace-scoped,与查询表达式完全一致)
...
**三条查询路径(按输入形态分流,写死;每条路径有对应索引,§5.2 逐条 EXPLAIN)**:
...
- **性能验收(§5.2 / README §10)**:§10 基准(10 万 issue / 1 万成员工作区,50 VU 稳态 + 100 VU 峰值,冷/热缓存各标注)下搜索 P95 < 300ms、identifier 快路径 P95 < 100ms;附三条路径各自的 `EXPLAIN (ANALYZE, BUFFERS)`,证明命中上述 trigram/pattern/唯一索引、**无全表顺序扫描**(1–2 字符路径同样必须走 `*_prefix` 索引,与本节 DDL 互为印证——不得互相否定;可见性 JOIN 走真实成员/项目表)。
...
| `cursor` | opaque | 整体游标(§6.14):**`cursor = base64(完整排序元组 + 绑定指纹 + 服务端 HMAC 签名)`**——完整排序元组与 §4.6 全序**逐因子一一对应,且全部为数据库可稳定计算因子**:`(score_bucket, title_len, title_lex, type, id)`(score 以固定精度量化为整数 bucket,杜绝浮点不可复现;**本地 recency/frequency/上下文不进元组**,R2-H4);绑定指纹 = `sha256(q \| 排序后的 types 集合 \| workspace_id)`,**cursor 与产生它的 query/types/workspace 绑定,跨任一参数复用 → `400 validation_error`**(防攻击者搬运 cursor 探测他查询/他工作区的 keyset 路径);签名不符 → `400 validation_error`;内部字段经签名校验前**一律不信任**(MES-75 安全收口延续) |
...
3. **本地 recent 失效清理(惰性)**:渲染空态时对 recents 条目做**轻量批量存在性/可见性核验**(批量 id 查询,失败/403/404 即过期),命中失效即从本地存储剔除;**清理以打开面板批量核验为准**(`favorites.changed` 虽已在 §6.7 登记,后端尚无发布点、前端不订阅,事件联动待频道形态具备后补);**被删/失权对象不得残留在 recents**;
...
- [ ] **命令条目(评审 P6 逐条覆盖核对)**:§1.2 S3 枚举清单**逐条**有测试核对——十组命令(顶层导航 × 按 §6.12 信息架构全集、设置各子页、待审批、新建 issue、主题 ×4、复制深链、收藏/取消收藏、标记全部已读、帮助层、重新显示上手清单)均可经面板执行且各有等价鼠标路径;**角色门控断言**:以 guest/agent 身份打开面板,设置各子页与危险操作命令**不注册不渲染**(非「点击报错」);以 admin 打开则齐全;「标记全部已读」随当前收件箱视图 filter 发送(comment-inbox.md `POST /inbox/read-all` 同 filter,e2e 断言请求体 filter 与当前视图一致)。
