...
| `stream_state`    | JSONB       | NOT NULL                                                                                              | `'{}'`              | **钉钉 Stream 连接状态持久真源**(MES-82;仅 `kind='im_dingtalk'` 且 `receive_mode='stream'` 使用):`{state:'connected'\|'reconnecting'\|'down', last_frame_at, last_attempt_at, backoff_seconds, reconnect_request_id?}`;由 Stream worker 在状态迁移事务内经 outbox 同步广播 `integration.updated(subject='stream_channel')`(README §6.6/§6.7);`reconnect_request_id` 是 `POST .../{id}:reconnect` 写入的内部不透明唤醒标记,不经状态诊断响应回显;API 与 worker 的所有 JSONB 合并写均先对该 integration 行 `SELECT ... FOR UPDATE`,再基于锁后最新值合并,禁止整对象陈旧覆盖丢失重连标记;`GET .../integrations/{id}/stream-status`(§3.9)读取公开状态,UI 首屏与诊断不依赖实时事件先达 |
...
| `visibility_scope`  | TEXT        | NOT NULL,CHECK IN ('workspace','project','unknown')                                                        | `'unknown'`         | **事件台账授权快照**:仅签名通过且归一化成功的事件可按服务端绑定真源写 `workspace`/`project`;未匹配、被拒、畸形、不可归一及迁移前历史行一律 `unknown` 并仅 admin/owner 可读。不得从外部 payload 猜测作用域                                                                       |
| `project_id_snapshot` | UUID      | NULL,无 FK                                                                                                 | NULL                | `visibility_scope='project'` 时必填的不可变项目快照;故意不建 FK,使审计行在绑定/项目删除后仍保留。非管理者读取仍要求该项目当前存活且对本人可见;项目已删时仅 admin/owner 可读                                                                                             |
...
**读取授权与分页顺序(MES-90 P0)**:`GET .../integrations/{id}/bindings`、`GET .../integrations/{id}/events` 与已连接列表的近 7 天事件量使用同一 SQL 可见性谓词。workspace 作用域对工作区成员可见;project 作用域必须命中当前存活的公开项目或本人项目成员/guest grant;`unknown` 与已删除项目快照仅 admin/owner 可见。谓词必须在 `COUNT`、排序和 keyset cursor/`LIMIT` **之前**进入 SQL,禁止先分页后在 Python/渲染层过滤(否则会产生短页、游标跳行与隐藏事件量侧信道)。绑定列表按 `integration_bindings.scope/project_id` 执行同一规则,不得向无项目权限者泄露 `external_ref`/匹配规则/目标 agent。

**入站绑定身份解析(MES-90 P0)**:事件可见性快照与后续触发路由必须使用同一完整外部身份键 `(provider, provider_tenant_key, external_ref)`，并同时限定当前 `workspace_id/integration_id`;不得仅按 `external_ref` 查找。集成租户配置轮换后，旧/新 tenant 下可合法存在同名外部对象，二者必须各自命中唯一绑定，不能产生多行查询异常或歧义抑制。disabled 绑定仍占据全局身份并作为隐私边界，但不触发运行。

**事件实时授权(MES-90 P0)**:`integration.event_ingested` 必须按同一不可变快照选择频道,不得统一广播后再依赖客户端过滤。合法 workspace 快照投递 `workspace:{workspace_id}:integrations` 与 `integration:{integration_id}`;合法 project 快照**仅**投递 `project:{project_id_snapshot}`(订阅鉴权复用项目读取契约);`unknown`、畸形快照与已拒事件不投递该事件帧。事件帧只能在摄取校验结束、确定最终非 rejected 状态后写入 outbox，禁止先发 `received` 再把台账改为 rejected。项目频道在订阅时、replay **每一行实际发送前**及每次 live 投递前都重新校验当前项目可见性，权限撤销后立即移除订阅并 fail closed，不能把一次页首鉴权复用于整页回放。无项目权限的成员不得从 workspace/integration 频道观察到私有项目事件的数量、时序、event_id 或状态变化。前端仅订阅服务端已过滤绑定所涉及的项目频道，并为绑定读取失败、频道上限或订阅错误启用定时 REST 对账与绑定重试，不能在 WebSocket 仍显示 connected 时永久漏刷新。
...
> **`config` 严禁存明文 secret**:一切密钥以服务端拥有的密文引用存储(同 `runtime_credentials.encrypted_value` 契约,README §6.16)。钉钉外部创建/更新配置采用**闭合字段集**，`app_secret` 只能经请求顶层 `secret` 或轮换端点提交；客户端提交 `app_secret_ref`（即使伪装成已有引用）一律 400 `invalid_request`，服务端加密后仅写 `integrations.secret_ref`。升级前已持有 `config.app_secret_ref` 的行只保留读兼容，下一次凭据轮换会移除该旧字段。`config` 仅存 app_id、外部租户标识、回调基址、模板等非密配置。钉钉配置必填非空且 trim 后的 `app_key`/`corp_id`；`receive_mode∈{stream,http}`、`inbound_queue∈{serial_conversation,parallel}`、`verbosity∈{final_only,progress}`；`stream_reconnect` 只允许 `base_seconds`/`max_seconds`/`heartbeat_timeout_seconds`，均须有限且大于 0，分别上限 300/300/3600 秒，且 `max_seconds≥base_seconds`。未知字段或未知枚举均 fail closed。**Stream 网关测试替身基址 `MESH_DINGTALK_GATEWAY_BASE` 亦不进 config**——仅部署期环境变量,不经任何运行期配置/管理 API 可读写(防 admin 可编辑 → 指向受信任对端 → Stream MITM 窃取内存中 app_secret 的提权路径),生产环境非默认值启动即告警 + 审计(§5.6)。

> **外部卡片站点基址(部署硬约束)**:`MESH_APP_BASE_URL` 必须是绝对 `http://` 或 `https://` URL（可带部署子路径，不得带用户名/密码、query 或 fragment，规范化时移除尾 `/`）。API 与 worker 均必须显式配置；它是审批卡片 `[回 Mesh 处理]` 深链的唯一 origin。缺失或非法时，卡片投放以 `invalid_configuration` 失败且不得调用钉钉；卡片回调在任何审批决定前返回 500 `integration_misconfigured`、审计 `app_base_url_missing`，审批真源保持不变，禁止退化为相对 URL。

> **钉钉 `app_key` 归属与共享组写入契约(迁移 0039)**:`app_key` 在全系统至多归属一个 workspace。首次认领必须在事务级 `dingtalk_app_owner:<app_key>` advisory lock 内，以候选 `app_key + app_secret` 对部署期固定的钉钉 OAPI 做无副作用凭据校验；平台拒绝 → 422 `dingtalk_credentials_invalid` 且不落行，平台不可达 → 502 `dingtalk_credential_verification_unavailable` 且不落行，故公开 app_key 不能以假 secret 首占。已归属另一 workspace → 409 `dingtalk_app_key_conflict`。同 workspace 共享 app 时，`(corp_id, robot_code || app_key)` 路由必须唯一(409 `dingtalk_route_conflict`)、所有兄弟必须解密为同一 app_secret(409 `dingtalk_app_credential_conflict`)，全部 Stream 兄弟还必须共享同一规范化 `stream_reconnect`(409 `dingtalk_stream_config_conflict`)。`rotate-secret` 与显式 `:reconnect` 均以 app 组为作用域，向全部相应兄弟写入同一密文/重连标记。升级 0039 对既有记录采用“已由运维信任”的兼容假设，但若同一 app_key 已横跨多个 workspace 则**中止迁移**，必须先由运维明确清理，禁止按 UUID/创建时间猜测所有者。
...
CREATE TABLE integration_events (
  ...
  project_id_snapshot UUID NULL, -- 不建 FK:删除后审计快照仍存;非管理者读取须命中当前可见存活项目
...
| POST | `/api/v1/integrations/dingtalk/events` | 钉钉/DingTalk | **HTTP 回调模式**(`config.receive_mode='http'`):请求头 `timestamp`(毫秒)+ `sign = Base64(HMAC_SHA256(app_secret, timestamp + "\n" + app_secret))`;恒定时间比较 + 时间戳防重放(**钉钉官方容差 ±3600s**,严于其上限即拒绝合法回调,不得收窄);签名前及签名校验均只考虑 `status='active'`、未删除、`receive_mode='http'` 的集成，必须经 body 非空的 `(chatbotCorpId, robotCode)` 与配置 `(corp_id, robot_code || app_key)` 完整二元组精确定位，缺失/错误/零候选均 401，禁止退化为 corp-only 或由 Stream 兄弟接收；`msgId` 作 `external_event_id`。**Stream 模式不经本端点**(Mesh 侧主动出连,见下)                                                                                                                               |
...
stream worker(常驻进程,与 outbox relay 同类的基础设施 worker)启动
  ...
  → 帧处理协议(帧结构 { specVersion:'1.0', type, headers{topic,…}, payload, time }):
     ...
     · type='CALLBACK' topic='/v1.0/card/instances/callback' → 以 V2 outTrackId 回查 approval/workspace
       及源 integration、corpId 精确落到该 app 的租户路由 → 卡片回调鉴权链(§3.10)；事务提交后 ACK data
       必须携带 §4.4 lifecycle writeback，403 权限拒绝同样 ACK 禁用卡片；解析/路由/服务错误
       不 ACK，交由平台重推，重复点击由统一 approval 真源幂等
...
>
> **Stream 当前真值边界**:物理连接建成时的内存快照不构成持续授权。每个消息帧与卡片帧都须在事务中 `SELECT ... FOR UPDATE` 重读其源 integration，要求未删除、`status='active'`、`receive_mode='stream'` 且 app/corp/robot 身份仍与当前路由一致。消息帧遇停用、切 HTTP、删除或身份漂移时写 durable `rejected` 台账且不入队；卡片帧遇同类失配时留拒绝审计、审批不变。禁用卡片是已认证终态拒绝并 ACK；解析失败、未知 topic/type、原始 JSON primitive 等协议错误须持久记录诊断且**不 ACK**，由平台重推。只有事务提交后的成功/幂等结果或明确 ACK-safe 的当前真值拒绝才可 ACK，禁止旧连接快照继续授权副作用。
>
...
>
> **单实例互斥与崩溃安全**:同一 `app_key` 的 Stream worker 以单个 PostgreSQL advisory lock(`pg_advisory_lock(hashtext('dingtalk_stream_app:'||app_key))`)对该应用的全部 active Stream 集成作 **all-or-none** 全局所有权,避免多个 worker 分片后各建物理连接;即便互斥失效,`integration_events.UNIQUE(integration_id, external_event_id)` 去重仍是最终幂等兜底。worker 崩溃由进程守护(compose `restart: unless-stopped`)重拉,重连后钉钉重推未 ACK 帧,不丢消息。取消/退避中关停必须等待内部 sleeper、连接任务与锁 session 全部清理后才返回,不得 fire-and-forget 释放锁。**平台连接上限(S-5)**:钉钉 Stream **单应用最多 50 条连接**——多个 Mesh 集成配置**共享同一 `app_key`** 时按 `(app_key)` 粒度共用一条物理连接,帧以载荷 `(chatbotCorpId, robotCode)` 与配置 `(corp_id, robot_code || app_key)` 的完整二元组精确定位;零个或多个候选一律 fail closed,禁止以裸 `robotCode` 覆盖路由或单集成回退。`corp_id`/`robot_code`/规范化 `stream_reconnect` 热更新均纳入连接指纹并先关闭旧组,确保旧路由与旧心跳策略不继续生效。每轮退避只生成一个抖动值，同一值同时写入 `stream_state.backoff_seconds` 并实际 sleep；开始立即连接/已连接时写 0。帧错误计数与节流按 app 组隔离，尾批在 frame loop 退出时强制刷入，不得被另一 app 的错误风暴抑制。`GET .../stream-status` 对共享连接的同 app 集成返回同一连接态。
>
...
| POST | `/workspaces/{ws}/integrations/{id}:reconnect`              | **显式重连(仅 active 钉钉 Stream 模式)**:返回 `202 {data:{accepted:true}}`;事务内把 `stream_state.state` 置 `reconnecting`、退避清零并写新的不透明 `reconnect_request_id`,同时经 outbox 广播 `integration.updated(subject='stream_channel')`。持有咨询锁的 worker 在协调扫描中以该持久标记作为连接指纹的一部分,即使 API 与 worker 不同进程也会结束当前连接/退避并重建;共享 `app_key` 时重建对应唯一物理连接。不得实现为前端本地状态切换或 `:test` 别名 | admin / `integration:manage`     |
...
| 审批/交互卡片                       | **钉钉互动卡片 card_1.0 体系(写死,勿与旧 `im/robots/interactiveCards` 混用;`sampleActionCard6` 等传统 ActionCard 无回调/无更新能力,仅可作纯通知卡,严禁承载 §6.10 审批卡)**:投放经 **`POST /v1.0/card/instances/createAndDeliver`** —— `cardTemplateId`(模板)+ **V2 `outTrackId`(每卡唯一,紧凑编码 `approval_id + source_integration_id`,≤64 字符，回调据此精确锚定源实例)** + **`openSpaceId`(`dtv1.card//IM_GROUP.<openConversationId>` 群 / `dtv1.card//IM_ROBOT.<senderStaffId>` 单聊)** + `cardData` + **`callbackType='STREAM'`(复用本模块已建长连接,推荐)或 `'HTTP'`**(独立回调地址,签名同 §3.2 钉钉行,二选一与接收模式对齐);更新经 `PUT /v1.0/card/instances`;卡片参数受官方硬约束(`cardParamMap` key≤100B / value≤1KB)。旧 approval-only `outTrackId` 仅为已投放卡兼容解析：同 corp/robot 恰好一个候选时可处理，兄弟实例歧义时 fail closed，禁止由 active sibling 接管已停用源卡 | 回调(topic `/v1.0/card/instances/callback`)鉴权链同飞书/Slack 卡片(§3.2/§4.3 流程 B):**点击者身份锚定回调载荷 `userId`(配 `userIdType`,**写死按 staffId 归一**),无 staffId 回落 `x=<base64url(senderId)>` 编码**(§3.10 编码,与入站同映射)→ `external_identities`(provider='dingtalk', tenant=corp_id)→ 全局 `users.id` → 集成解析 workspace → JOIN `members` 名册行 → README §6.10 权限校验 → 转发 `POST /approvals/{id}/approve\|reject`;未映射/无名册行/无权限 → 403,审批状态不变,留痕;**回调响应即回写卡片**(响应体 `cardUpdateOptions` + `cardData`/`userPrivateData`,loading 态经 `userPrivateData` 按点击者私有呈现,终态经 `cardData` 公共更新;§4.4 生命周期) |
...
- **钉钉连接状态卡(MES-82)**:Stream 模式集成概览顶部 —— 状态点(connected/reconnecting/down)+ 最近心跳相对时间("12 秒前")+ 接收模式徽章;`down` 时横幅"Stream 长连接断开,自动重连中(退避 Ns),期间消息将由钉钉侧暂存并重推"。admin/owner 的异常态同时提供彼此独立的 **[重新连接]**(调用 `:reconnect` 持久唤醒 worker)与 **[编辑配置]**;普通成员不渲染重连写操作。**[测试发送] 与 [诊断接收] 分离两个动作**:[测试发送] 经 OpenAPI 出站(不依赖接收信道,失败提示 `upstream_error`/凭据问题,不误导为连接故障);[诊断接收] 读 `stream-status`(接收信道态)。HTTP 模式展示只读回调 URL + [复制],无连接状态([测试发送] 同样可用)。
...
> **前端边界实现注记(MES-90)**:钉钉创建/编辑表单对白名单非密字段结构化提交(`app_key`/`corp_id`/`receive_mode`/`inbound_queue='serial_conversation'`/`verbosity`/`ack_template`),`app_secret` 只走顶层 `secret`/轮换端点,不得进入 JSON config。队列面板只渲染服务端授权切片,本人取消资格以当前用户 `external_identities` 的完整 `(provider, provider_tenant_key, external_user_key)` 三元组匹配 `sender.identity_key`;不得以裸 user key 比对。workspace 级 `queue_updated` 携 `conversation_key` 时只重拉该会话;project 级失效通知不携会话键时重拉完整授权分片。孤儿审计入口仅 admin/owner 可见且按需请求。Mesh 侧卡片预览只演示审批 API 真源:用户选择具体 approval 后读取 `GET .../approvals/{id}`,响应的 `execution_status` 必须来自其关联 `task_executions` 当前状态而非路由常量或前端推断;pending 时每 4 秒自动对账直到终态,回 Mesh 链接固定携 `approval_id` 打开同一审批详情;预览不持有或改变审批决定,不得提供本地状态下拉伪造卡片生命周期。
...
> **钉钉卡片更新协议要点(card_1.0)**:状态流转经卡片更新接口以 V2 `outTrackId`(由 `approval_id + source_integration_id` 派生)幂等更新并避免共享 corp/robot 兄弟歧义;点击瞬间的 loading 经回调**响应体** `userPrivateData`(仅点击者可见)回写,终态文案经 `cardData` 公共更新并禁用按钮;**若未来采用 AI 流式卡片**(长结果流式呈现,非本期审批卡片路径),须遵守官方约束:markdown `isFull=true` 全量更新、单帧 ≤1KB / 总量 ≤3KB、`guid` 幂等,且**终态以 `UpdateCard(flowStatus=FINISHED/FAILED)` 显式收尾**(不仅依赖流式 `isFinalize`)。
...
- [ ] `integration.updated`(集成/绑定/订阅变更、熔断)与 `integration.event_ingested`(入站落库)均取自 README §6.7 注册表「平台能力」域,无未登记事件名;经唯一写入路径推送,带频道 `seq`,断线凭 `resume_from` 重放无丢失无重复(README §6.7/§9 T26)。事件入站帧严格按 `visibility_scope/project_id_snapshot` 路由:workspace 快照走 workspace+integration,project 快照只走项目频道,unknown 不发;私有项目 outsider 的真实 WS 负向断言不得收到 `integration.event_ingested`。
...
- [ ] **单实例互斥与 app_key 原子分组**:两个 worker 进程同时启动 → 以 `app_key` 为粒度争抢单个 session advisory lock；共享该 app_key 的全部 active Stream 集成必须由一个 worker **all-or-none** 持有并共用唯一物理连接，禁止按 integration_id 分锁后被两个 worker 各持子集。新增/删除/轮换/reconnect 先关闭旧组并释放 app-key 锁，再由任一 worker 原子取得完整新组；集成测试用两个 manager/独立数据库 session 断言一方取得全组、另一方为零，互斥失效时仍由入站去重键兜底无重复执行。
- [ ] **共享连接路由与可等待清理**:同 app_key 下构造相同 `robotCode`、不同 `chatbotCorpId` 的两个集成,帧必须按完整二元组只落入目标集成；同二元组重复或任一段缺失则不落事件、不匹配绑定、不触发运行。运行中修改 `corp_id`/`robot_code` 必须触发旧 socket/路由表重建。连接在最长退避中被 reconnect/shutdown 取消后,内部 stop waiter/sleeper 与咨询锁 session 均已完成清理,`shutdown()` 返回后不得残留 run-owned task 或锁。
- [ ] **app 归属与组真值**:假 app_secret 的首次认领不得落库或阻止真实所有者；跨 workspace 同 app_key 返回 `dingtalk_app_key_conflict`。同 workspace 重复路由、不同 secret、不同 Stream reconnect 策略分别按具名 409 拒绝；组级 rotate/reconnect 更新全部兄弟。0039 遇既有跨 workspace 冲突必须中止，升级/降级后 SECURITY DEFINER 函数权限符合最小授权。
- [ ] **Stream 卡片真实链路**:真实 worker + 真实 TLS/WSS fake gateway 下发送 `/v1.0/card/instances/callback`，统一 approval 真源落 approved/rejected，ACK data 为 lifecycle card body；403 不改审批但 ACK forbidden body；同帧重推幂等。解析/租户/approval 路由错误不 ACK。
...
- [ ] **卡片 API 面(C-3)**:审批卡片投放经 `createAndDeliver`(断言请求含 `cardTemplateId`/V2 `outTrackId`=approval+源 integration 紧凑派生且 ≤64 字符/`openSpaceId` 群或单聊格式/`callbackType`);**审批卡片路径不出现 `sampleActionCard6`/传统 ActionCard**(代码路径断言);更新经 `PUT /v1.0/card/instances`,outTrackId 幂等(重复更新不冲突)。
...
- [ ] **出站请求体零日志**:`connections/open`/`accessToken` 出站失败(模拟 5xx)→ 日志/错误台账仅 `method/url/status`,请求体、响应体及鉴权头值均不记录;`redact_in_logs` 黑名单含长期凭据的解密值。
