...
> **钉钉 `app_key` 归属与共享组写入契约(迁移 0037)**:`app_key` 在全系统至多归属一个 workspace。首次认领必须在事务级 `dingtalk_app_owner:<app_key>` advisory lock 内，以候选 `app_key + app_secret` 对部署期固定的钉钉 OAPI 做无副作用凭据校验；平台拒绝 → 422 `dingtalk_credentials_invalid` 且不落行，平台不可达 → 502 `dingtalk_credential_verification_unavailable` 且不落行，故公开 app_key 不能以假 secret 首占。已归属另一 workspace → 409 `dingtalk_app_key_conflict`。同 workspace 共享 app 时，`(corp_id, robot_code || app_key)` 路由必须唯一(409 `dingtalk_route_conflict`)、所有兄弟必须解密为同一 app_secret(409 `dingtalk_app_credential_conflict`)，全部 Stream 兄弟还必须共享同一规范化 `stream_reconnect`(409 `dingtalk_stream_config_conflict`)。`rotate-secret` 与显式 `:reconnect` 均以 app 组为作用域，向全部相应兄弟写入同一密文/重连标记。升级 0037 对既有记录采用“已由运维信任”的兼容假设，但若同一 app_key 已横跨多个 workspace 则**中止迁移**，必须先由运维明确清理，禁止按 UUID/创建时间猜测所有者。
...
- [ ] **app 归属与组真值**:假 app_secret 的首次认领不得落库或阻止真实所有者；跨 workspace 同 app_key 返回 `dingtalk_app_key_conflict`。同 workspace 重复路由、不同 secret、不同 Stream reconnect 策略分别按具名 409 拒绝；组级 rotate/reconnect 更新全部兄弟。0037 遇既有跨 workspace 冲突必须中止，升级/降级后 SECURITY DEFINER 函数权限符合最小授权。
