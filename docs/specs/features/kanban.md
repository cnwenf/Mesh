...
看板遵循 design-quality.md §11 的 Mesh 适配层边界：视图切换、筛选/分组、列头、卡片动作、紧凑模式和触控移动面板统一复用共享 Button/Input/Select/Badge 等组件；feature 不直接依赖底层组件库。桌面拖拽、键盘移动与触控移动必须继续汇入同一原子 move 命令，迁移不得改变 WIP、乐观回滚、实时收敛或 `/w/:workspaceSlug/views/:viewId` 深链契约。视图列表、投影、创建/move 与 Realtime 频道只使用该路由下 `WorkspaceProvider` 的当前工作区；不得从 membership 顺序猜测作用域，动态路径段须 URL 编码，工作区/视图切换后的旧请求不得回写当前看板。
