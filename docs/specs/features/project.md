...
项目列表与详情遵循 design-quality.md §11 的 Mesh 适配层边界：筛选、创建/编辑表单、健康度动作、页签、状态徽章和反馈态复用共享设计组件；项目 feature 不直接依赖底层组件库。所有卡片、创建成功、设置和删除回跳必须保留 `/w/:workspaceSlug/projects...` 规范深链，并按路由 slug 选择 membership，禁止多工作区错配。
