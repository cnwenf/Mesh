...
- **鉴权前置(安全契约)**:预览与未确认(`confirm` 缺省)请求携带完整字段清单,**先鉴权、后出清单**:源 issue 走读门(不可见 → 404 `not_found`;guest 与其他成员**同口径**——LOW-S2 anti-oracle 收敛,私有项目存在性不外泄,与 `GET /issues/{id}` 同矩阵);`target_project_id` 非空时走项目写门(不可见 → guest 404 / 403,不存在/跨工作区 → 404)。任何鉴权失败**只回错误信封,不携带 `preview`**。`POST /issues/bulk` 未确认聚合预览同理**逐条**过源 issue 读门:越权项仅回 error marker(`{"issue_id", "error": "not_found"}`),**不回 plan**;目标项目写门在聚合前整体校验(失败即整体 403/404)。**聚合预览覆盖全部条目**(`issue_ids` 上限 100,不截断),确认前每一项的映射/清除清单可见。
