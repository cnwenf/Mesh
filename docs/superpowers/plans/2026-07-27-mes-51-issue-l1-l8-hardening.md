...
- 代码/注释/文档/提交信息/分支名不得暴露任何外部出处（无竞品名、无 platform 字样）。
...
- [ ] **Step 2: CHANGELOG 新块**（仿既有版式，记 MES-51 L1–L8 + NOTE，不出现外部出处字样）
...
`platform issue comment add` 发回 MES-51：结果 + PR 链接 + @Mesh 验收员（mention 约定见 platform-mentioning skill）。
