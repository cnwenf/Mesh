...
MES-130 Step 2 批次②收口与看板二维泳道(MES-159 / MES-173 / MES-130,PR #128/#129/#132/#133)——核心工作区页族(项目/issue/看板)完成 Appica 底座迁移并修复 IssuesPage 多工作区作用域缺陷(锚定路由 slug);看板泳道协议(sub_group_by)Spec 定稿,二维泳道运行时落地(投影/分组/渲染/交互,斜向拖拽刷新后真实持久化);接口保真基线逐页收敛(OKLCH 语义令牌表、256px rail + 8px 外缝 14px 圆角内容框、Inter 密集字阶、中性低对比选中态、语义淡色列,亮暗双主题 + 移动端 AA 对比度修复)。验收闭环:批次②见 MES-174/175(泳道除外),泳道运行时与保真基线随 MES-130 收口一轮验收(2026-08-04T20:54Z 通过)+ 一轮安全复审(22:10Z 通过,无 CRITICAL/HIGH)。前端单测 417 文件 / 4668 例全绿,整体 lines/statements 98.9%、functions 97.97%、branches 94.29%,逐变更文件四指标 ≥90% 门禁不回退;真实栈旅程 40/40(含泳道 7×N 矩阵与拖拽落库);CI 14/14 门禁全绿。competitor-parity-checklist 316 条已勾销 193 条,剩余待实机/部分项由 MES-185 收口。

### Added

- **MES-130 Step 2 批次②:核心工作区页族迁移(MES-159,PR #128)**:项目列表与详情、issue 列表与详情、看板迁移至 Appica 组件底座与 Mesh 设计 token,规范工作区路由、权限与空/加载/错误态;一维看板与泳道分组渲染就绪;配套 MES-173 泳道协议 Spec 评审文档(协议定义 + quick-create 权限/过滤缺口补全)。
- **看板泳道协议 Spec(MES-173,PR #129)**:sub_group_by 二维泳道协议定稿——投影/分组/交互契约作为运行时实现前置,评审意见已全部落文档。
- **二维泳道运行时(MES-130,PR #132)**:在协议之上实现二维泳道全链路(后端投影分组 + 前端渲染交互),真实栈验证泳道矩阵渲染、斜向拖拽后刷新仍在目标格(DB 持久化)、移动端 lane-tabs 降级。
- **接口保真基线(MES-130,PR #133)**:全前端逐页对齐设计基线——OKLCH 令牌表、rail/内容框几何、字阶/间距/圆角收敛,visual 基线 424 例全绿,396 张真实环境证据截图;design-quality.md / interface-design-baseline.md / README 同步新基线。

### Security

- **cryptography CVE-2026-69247 清零(PR #129)**:升级至 50.0.0,三份 lockfile 同步;pip-audit runtime/dev 均零已知漏洞。

### Fixed

- **IssuesPage 多工作区作用域缺陷(PR #128)**:作用域取值由 memberships 首项改为锚定当前路由工作区 slug,与项目页 slug 作用域不变量对齐,消除跨工作区数据错位/暴露面。
- **移动端收件箱对比度(MES-130,PR #133)**:恢复 AA 对比度。
...
- **设置 + 搜索/命令面板 + Analytics + 审批 前端批次④(MES-111 / MES-127,design-quality.md §3.2/§4.4/§9.6/§11.4 + search-command-palette.md 全条 + analytics.md §4 + competitor-parity G3/G4/G10/G11/G19)**——
  ...
  - **验证**:单测 3856 例全绿、逐文件 90% 门禁(189 文件)通过;后端搜索真实 e2e 44 例全绿;前端批次④真实 e2e 桌面 + 手机双视口 10/10 全绿 + 四组合走查存证 22 张互异(`e2e/evidence/mes111-b4/`);lint/typecheck/build/对比度/视觉回归(38)全绿。
...
- **集成·钉钉双接收摄取(MES-87,integrations.md §3.2/§3.7/§5.6)**:钉钉企业内部应用机器人**双接收模式**落地,两种模式仅替换鉴权适配层、其后共享唯一摄取核心 `ingest_verified_event()`(§2.10:651-664 摄取分层边界);命令平面 / 签名后护栏 / 会话队列复用 MES-88 发布实现(`commands.maybe_handle_command` / `inbound_guards` fail-closed / `message_queue.enqueue_message` 含锁内 pending 深度权威复检),本切片不另起并行实现:
  - **HTTP 回调适配器**:`timestamp`+`sign` 校验(`Base64(HMAC_SHA256(app_secret, timestamp+"\n"+app_secret))`、`hmac.compare_digest` 恒定时间比较、官方 ±3600s 容差以 `max()` 保证不收窄——签名串不覆盖 body);`chatbotCorpId`(+`robotCode`)定位;签名无效/缺失/重放一律 401 + `rejected:` 审计命名空间,绝不分发;签名**前** (集成,IP) 120/min 滑窗(auth.md §3.6:超限**静默 200** + 审计告警,集成不可定位退化 IP 维);1MiB body 上限。
  - **Stream 长连接适配器**:`mesh.workers` 受监督任务(不新增 compose service);`connections/open`(clientId/clientSecret/subscriptions/ua → endpoint/ticket)→ **wss-only + 强制证书校验**(非 wss 拒连 + 告警,禁 verify=False;ticket 为平台协议强制的短期一次性凭据——§6.16 URL-query 禁令的显式命名例外);帧协议 `specVersion='1.0'`:SYSTEM ping 必回 ACK(200 + 原 headers 原样 + 原 payload data)、disconnect 主动关闭立即重连、CALLBACK 消息帧经**同一摄取核心**(`signature_status='valid'`,`payload._mesh_channel='stream'`)提交后 ACK `received`(未 ACK 重推经 msgId 幂等);`pg_try_advisory_lock(hashtext('dingtalk_stream:'||integration_id))` 单实例互斥 + 共享 app_key 按 app 一条物理连接(平台上限 50,帧按 robotCode 路由);指数退避 2→300s ±20% 抖动;心跳 90s 无帧重连;`stream_state` 持久真源 + outbox `integration.updated(stream_channel)` 广播;`GET .../stream-status` 接收诊断只读端点;凭据轮换/停用/删除经 scan 对账;`MESH_DINGTALK_GATEWAY_BASE` **仅部署期环境变量**(不进 config/管理 API),生产非默认值启动即审计告警。
  - **共享核心与归一化**:`VerifiedEnvelope` 分层边界;`conversation_key` 三段(external_ref 字符类 `^[A-Za-z0-9_.@+/=-]+$` 不含 `:`,官方样例 `cid…==` 合法);身份段 staffId 原值(最宽字符集 `[A-Za-z0-9._-]`)或 `x=<base64url(senderId)>`(第 2 字符恒 `=`,字符集代数保证与 staffId 键空间结构不相交,含冒号原值拒绝);msgtype 矩阵(群 @ 仅 text/richText/picture 投递、**触发仅 text**、非文本仅审计 `processed` 不触发/不入队/不 ack);@前导空格 trim;`MESH_IM_INBOUND_TEXT_MAX_CHARS`(4000)字符截断 + `truncated` 审计(HTTP/Stream 双通道同口径);命令平面经 MES-88 `commands.maybe_handle_command`(/stop·/btw·/help,未知命令回帮助文本,不入队不触发)。
  - **签名后语义护栏(§2.10,复用 MES-88 `inbound_guards`)**:身份 20/min 全局跨会话、会话 60/min(Redis 滚动窗口含租户维)、pending 深度 50(摄取事务预检 + `enqueue_message` 咨询锁内权威复检);**fail-closed**:Redis 故障异常上抛、摄取事务回滚(平台重推、去重保安全),绝不静默放行;超限不入队/不执行/不 ack——`rejected` + `payload._mesh_reject_reason='rate_limited'` + 真实 msgId 占去重键(重试幂等)+ 同会话提示自身限频 ≤1/min(防提示反射,发射端自携 `conversation_type`/单聊目标键,MES-122 同口径)。
  - **IMQ 入队与 R5-2 契约**:经 MES-88 `message_queue.enqueue_message` 同事务入队——`imq_seq:` 咨询锁 + `ack_window_at=clock_timestamp()` 锁序时间 + `seq=max+1`(冲突重试 ≤3);`dispatch_mode` 快照(排空-再切换);各快照列;§3.8 ack leader 锁序窗口判定(leader 自指 + `im.send` outbox `sha256(queue_item_id|'ack')`,follower 仅指向不写事件,`ack_template=''` 全跳过);parallel 乐观直派;`integration.queue_updated` 失效通知双形(project 级不含 conversation_key)。`execution.enqueue` 消费者仅 `trigger='integration'` 附加 FOR UPDATE 队列项守卫(同租户过滤 + `dispatching AND execution_id IS NULL`)再建执行同事务绑 `execution_id` + 转 `processing`(§3.9 单一 dispatching→processing 转换),其它触发零回归。
  - **验证**:新增单测 ~110 例(签名正/负/±3600s 边界、归一化/键编码不相交、摄取去重/匹配/命令平面接线/护栏 fail-closed/入队/ack leader、Stream 帧协议/互斥/共享连接/退避边界/崩溃重建、R5-2 契约)+ **13 项真实 e2e**(真实 API + 真实 `python -m mesh.workers` 进程 + TLS 假网关——测试 CA 经 `SSL_CERT_FILE` 走**真实证书校验**):HTTP 签名/重放/去重/非文本/静默200;Stream 建连凭据、消息帧摄取+ACK、ping 原样回显、重推 msgId 幂等、disconnect 重连、stream-status 真源。`websockets` 提升为运行时直接依赖。
...
- **前端设计优化批次②验收修复(MES-125 第 2 轮,design-quality §9.4/§9.8/§11.4)**:验收打回的 4 项必修 + Low 项同轮清偿——
  - **List 布局数据竞态(必修 1)**:BoardPage 投影加载加 loadSeq + 视图 id 双重过期校验(切换视图后旧视图在途/分页响应一律丢弃);视图加载键(id + 投影配置)门禁杜绝 views 列表 refetch 换引用触发的多余旧视图请求;实时帧处理加视图 id 守卫(视图切换后晚到帧跳过)。新增 BoardPage.listViewRace 单测(在途旧响应不覆盖新视图)+ e2e 回归(新视图投影响应收敛 + 切换落定后旧视图 issues 端点零请求)。
  - **虚拟化真实布局失效(必修 2)**:看板列容器/列/列体补确定高度链(视口派生定高 + flex 收缩 + 内滚),虚拟化滚动容器 flex 高度解析为真实像素;卡片经 CSS 约束等高(单行截断);AT 坐标(aria-setsize/posinset)迁移至卡片本身,消除 listitem>listitem 双重嵌套;紧凑模式同链。新增 e2e 真实布局回归:经真实 API 造 210 卡(分两段避开 120/60s 写限流),断言内滚成立(scrollHeight > clientHeight)、窗口化仅渲染部分、滚动后渲染窗口迁移。
  - **execution.failed 未消费(必修 3)**:占位五态迁移——执行生命周期帧(started/awaiting_approval/failed/timeout/completed/cancelled)发布于 execution:{id} 自身频道,useCommentsData 按占位逐一订阅消费:failed/timeout → 失败占位(原因 + 重试入口,§9.8/comment-inbox §4.1)、waiting → 等待确认、completed/cancelled → 移除;重试经取回触发评论原文重发重新入队执行。
  - **Low 项**:落点指示条随 hit.index 移位(非恒列顶);乐观失败「动画回原位」(浮层 returning 态经 motion 过渡滑回源卡后清除,§9.4.4);视图删除二次确认对话框(§13.3);移除无引用文案键 board.listPlaceholderTitle/Description/quickAddDisabled;创建成功 1.2s 插入高亮(§9.3.4);快速创建 Cmd/Ctrl+Enter 与 Esc(§9.3.3);字符图标 ★/▸/▾/⟳/↺/+/− 全部替换为设计图标集(新增 star/rotate/rotate-reset/minus,§13.2)。
  - **分支同步**:rebase 最新 main(批次① PR #101 已合入)解决冲突。
...
- **验收返工(实测取证复核,CRITICAL/HIGH/MEDIUM 全收口)**:
  - **后端 CRITICAL**:`_message_attachments` / `distill_preview` 读路径此前直读 `attachment.mime_type/byte_size/scan_status`(模型无此属性)→ 带附件会话 `GET messages` 与沉淀预览恒 500;改 JOIN `attachment_blobs` 取快照字段,`distill_preview` 裸 SQL 换 ORM 并补「链接到本会话消息」可见性门;新增「发送→`list_messages` 往返」回归单测。
  - **前端 HIGH**:`real-members.spec.ts` 行操作 testid 迁 Menu(经 `member-row-*` 行内 menuitem 文案点击);`presence.ts` 字面 NUL 字节改 `（六字符转义序列：反斜杠 + u + 四个零）` 转义(git 不再当二进制);`ContextBar` 收起被作者 `display:flex` 盖过 UA `[hidden]` → 补 `[hidden]{display:none}` 兜底;`ConversationPanel` 补 `key={session.id}` 杜绝跨会话流泄漏;收件箱深链 `unknownId` 不再误报「源删除」+ 裸 UUID 标题,改「未找到/已归档」缺失态,归档当前选中通知清选中;手机 `/chat/:badId` 死胡同 → 确证失效后 `replace` 回 `/chat`。
  - **前端 MEDIUM/LOW**:`useAgentPresenceMap` 名单变化清空映射(防陈旧运行态);平板/粗指针行内控件命中区 ≥44px;底座 `Menu` 关闭归还焦点到触发钮;`useChatStream.abort` 本地置 `interrupted`(防停止竞态流式幽灵);铃铛拉取移出 setState updater + 下拉改 `role=region`(修正非法 menu 结构)+ 触发器 `aria-controls`;免打扰横幅分钟级 tick;收件箱错误态优先于骨架;乐观操作失败回滚 + danger toast(不再静默吞错);选中行 `aria-current` + `<time dateTime>`;清死键 `members.col.contact`;chat.css 魔法数换 token;已选行重复点击不再 push 重复历史。
  - **CI/存证**:消重 main 既存跨批重复 `phone-home-dark.png`(`mes111-shell` 与 `mes111-foundation` 同 md5)→ 单一存证于 foundation,evidence-unique 门禁转绿;手机重生成步不再静默跳过,四组合存证补齐至 58 张全互异。
...
- **前端设计系统底座(MES-111 Phase 1,design-quality.md §5–§9)**:按设计优化 Spec 落地逐页工作所依赖的设计系统基础层——
  ...
  - **组件状态矩阵**:Button 重做(sm 28/md 36/lg 44 三档,primary/secondary/ghost/danger 四变体均覆盖 default/hover/focus-visible/pressed/loading/disabled)、表单控件(hover/focus 环/disabled/readonly,错误经 aria-describedby 关联)、Icon(20px 网格线性 SVG 集 30 枚,描边 1.75 圆角端点,仅 16/20/24 三档,禁 emoji/字符图标)、Badge(图标+文案,颜色非唯一信号)、Avatar(人类姓名缩写 + 稳定 hash 八组配色;agent 统一轮廓)、Tooltip(hover 延迟/focus 即时,aria-describedby 关联触发元素)、Menu(role=menu 键盘漫游 ↑↓/Home/End/Esc + 外部点击关闭)、Tabs(ARIA tabs + 漫游 tabindex)、Accordion(渐进披露,aria-expanded/controls)、Drawer(桌面右侧滑入/手机自动转底部 sheet,焦点圈养 + Esc + 遮罩关闭);Dialog/Drawer/Menu 共用 useFocusTrap 焦点圈养实现。
  - **错误态/空态模板化**:ErrorState 升级为 §7.7 四部分(发生了什么/影响与输入保留/可执行恢复动作/可复制诊断 ID),EmptyState 补帮助链接插槽,杜绝「网络错误」式误导报错。
  - **forced-colors 扩展**:新增令牌全部重映射系统色(Canvas/CanvasText/GrayText/LinkText/Highlight),头像强制色归一。
  - **验证**:单测 3019 例全绿,src/design/ 逐文件覆盖率 ≥90% 门禁通过;对比度/硬编码色值/令牌幂等门禁全绿;真实浏览器 e2e 全过(含 MES-111 手机可达性 15 项);双主题视觉基线重生成(6 核心页 × 亮暗 × 桌面/平板);桌面 + 手机 × 亮暗走查存证(e2e/evidence/mes111-foundation/)。页面级逐页落地(登录/首页、看板/详情/评论、成员/收件箱/聊天、设置/搜索/分析、移动适配收尾)经 MES-111 并行子 Issue 分批实施。
...
- **集成·钉钉出站与互动卡片(integrations.md §3.8/§3.10/§4.4,MES-89)**:钉钉企业内部应用机器人出站适配器——`accessToken` 多副本单飞刷新(Redis 共享缓存 TTL=7200−300±60s 抖动 + 随机 owner token `SET NX EX 30` 租约 + Lua 条件释放 + follower `MESH_TOKEN_FOLLOWER_WAIT`(12s)有界双检等待;`token_refresh_busy` 为可重试非终态,仅后移 `available_at` 不耗失败预算;仅 `invalid_credentials`/刷新超预算终态);群 `groupMessages/send` / 单聊 `oToMessages/batchSend`(13 官方 msgKey;`msgParam` ≤15000B、无 @);限流错误码分类退避(`send.too.fast` 等 + `flowControlledStaffIdList`);外部联系人单聊 `no_staff_id` 降级(键编码 `x=<base64url(senderId)>`,与 staffId 键空间结构不相交)。ack 确认接收(§3.8):入队事务内按锁序 `ack_window_at` leading-edge 选主(5s 窗口 follower 不写事件、leader T2 批量回写 `ack_represented_at`/`ack_merged_into`;`ack_template=''` 跳过全部 ack 处理不占窗口);at-most-once 快 relay(T1 持久化 `ack_attempted_at` 闸门 + 事件终态化同事务提交 → 事务外 3s 超时外呼 → T2 回写 `ack_sent_at`;崩溃点三段可审计,失败不重试仅留痕)。互动卡片(card_1.0):`createAndDeliver`(cardTemplateId + approval 派生 `outTrackId` + `dtv1.card//IM_GROUP.<cid>`/`IM_ROBOT.<staffId>` openSpaceId + cardData + `callbackType` STREAM 优先)/ `PUT card/instances` 幂等更新 / `card/streaming`(guid 幂等、`isFull=true`、`isFinalize` 收口);**审批卡路径严禁 `sampleActionCard*`**(代码断言);回调鉴权链(userId 按 staffId 归一 → `external_identities` → `users.id` → 工作区名册 JOIN → §6.10 权限校验;未映射/无名册/无权 → 403 + 审批不变 + 审计留痕;重复点击幂等)与 §4.4 生命周期状态表回写(loading 私有态 / 终态禁用按钮 / 过期与失败附 [回 Mesh 处理] 深链);HTTP 回调端点 `POST /api/v1/integrations/dingtalk/cards`(官方 ±3600s 签名容差)。verbosity 与长文(§3.3/§3.10):默认 `final_only`(IM 仅确认/卡片/最终结果,`progress` 才推中间进度,站内恒为真源);结果超 15000B markdown 分段(分段幂等键 `sha256(notification_id|'chunk'|i)`,超 `MESH_IM_MAX_CHUNKS`(5)截断 + 站内深链);`notification.fanout` 后链式派生 `notification_delivery(channel='im', provider='dingtalk')` 台账 + `im.send` 事件。诊断分离(§3.9/§3.5):`POST .../test-send` 经 OpenAPI 出站(失败仅 502 `upstream_error`,接收信道 down 不影响、绝不返回 503);`GET .../stream-status` 只读 `stream_state`(`state='down'` → 503 `stream_channel_unavailable`,该码仅此端点出现)。`mesh.workers` 新增受监督任务 `im-send-relay`(`im.send` 唯一消费方,通用 outbox relay 仅领取已注册事件类型)。凭据(§6.16):app_secret 仅密文引用,解密值登记 `redact_in_logs` 黑名单;accessToken/appSecret 永不回显响应/日志(出站失败仅 method/url/status)。单测:新增模块覆盖率 ≥90%(ack 100%);真实 e2e:真 API 服务 + 真 worker 进程 + 本地 OpenAPI 测试替身,§5.6 出站/卡片断言逐条(双副本令牌单飞恰一次刷新、ack leading-edge 恰一条、分段投递、限流退避不耗预算、卡片回调全鉴权链、诊断分离、凭据零日志)。
- **平台错误类**:`UpstreamError`(502 `upstream_error`,出站平台调用失败,integrations.md §3.5)。

### Fixed

- **前端:HTTP 非安全上下文下登录及一切写请求失败(MES-129)**:`crypto.randomUUID()` 仅安全上下文(HTTPS / localhost)可用,HTTP 部署(`isSecureContext=false`)下为 `undefined`——API 客户端为所有 POST/PUT/PATCH/DELETE 自动携带的 `Idempotency-Key` 裸调它,抛 `TypeError` 使 fetch 根本不发出,前端归一为 `error.network`(「网络错误,请检查网络连接后重试」);GET 不带幂等键,故页面能加载而登录及全部写操作失败。修复:新增安全上下文无关的 `uuidv4()`(`frontend/src/api/uuid.ts`,优先原生 `randomUUID`,缺失时以任意上下文可用的 `crypto.getRandomValues` 按 RFC 4122 构造 v4),统一替换 `client.ts` 与聊天/评论/导入/导出四处裸调,并收敛附件上传器已有的同款内联兜底(消除重复实现)。验证:`uuid.ts` 单测两分支 + 掩码位逐位 + 千次无重复,覆盖率 100%,3025 例全绿(整体 lines 97.79%);真实 e2e(`playwright.mes129.config.ts`)经本机 LAN IP 以 HTTP 打开(前置断言 `isSecureContext=false`、`randomUUID=undefined`)对真实后端走通注册/登录 → 建工作区 → 建 issue → 发评论并 psql 校验落库;回归复现模式(`MES129_EXPECT_BUG=1` + 修复前源码)确认修复前登录报网络错误、停在登录页。
...
- `schema_r2_validation.sql` DDL 与行为验证(PG16)继续全绿;`docker compose up --build` 一键可跑(冒烟:建区 → 改名重定向 → 邀请创建/预览/接受/用尽 → 跨租户 404 → 角色变更审计,全部通过)。
...
- `schema_r2_validation.sql` DDL 与行为验证(PG16)随 main CI 持续通过;main 三 job 全绿。
...
- **CI 流水线**:`backend-ci` 三个 job——文档词汇/结构校验、单测 + 真实 e2e(真实服务进程/真实 API 调用/真实落库,pytest-cov ≥90% 门禁,ruff)、`schema_r2_validation.sql` 在 PostgreSQL 16 一次性实例实跑。

### Quality

- 单测 + 真实 e2e 共 150 项全绿,pytest-cov 95.34%(≥90% 门禁,整体与新增代码双达标)。
- `schema_r2_validation.sql` 在 PostgreSQL 16 实跑:断言全部 PASS、退出 0。
